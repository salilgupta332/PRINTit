...
export default function PANCardPrintService() {
  ...
  const handleSubmit = () => {
    ...
    toast({ title: "Order Submitted!", description: "Your PAN card print order has been placed." });
    setForm(initialForm);
    setStep(0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><Label>Full Name *</Label><Input value={form.fullName} onChange={(e) => update("fullName", e.target.value)} placeholder="Enter full name" /></div>
          <div><Label>Mobile Number *</Label><Input value={form.mobile} onChange={(e) => update("mobile", e.target.value)} placeholder="Enter mobile number" /></div>
          <div><Label>Email (Optional)</Label><Input type="email" value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="Enter email" /></div>
          <div><Label>PAN Number *</Label><Input value={form.panNumber} onChange={(e) => update("panNumber", e.target.value.toUpperCase())} placeholder="ABCDE1234F" maxLength={10} /></div>
        </div>
      </div>
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-3">📁 Upload PAN Card Image</h3>
        {form.uploadFile ? (
          <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/30">
            <span className="text-sm text-foreground flex-1 truncate">{form.uploadFile.name}</span>
            <Button variant="ghost" size="icon" onClick={() => update("uploadFile", null)}><X className="h-4 w-4" /></Button>
          </div>
        ) : (
        ...
        <div className="space-y-4">
          <div>
            ...
            <RadioGroup value={form.printType} onValueChange={(v) => update("printType", v)} className="flex gap-4 mt-2">
              <div className="flex items-center gap-2"><RadioGroupItem value="color" id="pan-color" /><Label htmlFor="pan-color" className="cursor-pointer">Color Print</Label></div>
              <div className="flex items-center gap-2"><RadioGroupItem value="pvc" id="pan-pvc" /><Label htmlFor="pan-pvc" className="cursor-pointer">PVC Card Print</Label></div>
            </RadioGroup>
          </div>
          <div>
            <Label>Lamination</Label>
            <RadioGroup value={form.lamination} onValueChange={(v) => update("lamination", v)} className="flex gap-4 mt-2">
              <div className="flex items-center gap-2"><RadioGroupItem value="yes" id="pan-lam-yes" /><Label htmlFor="pan-lam-yes" className="cursor-pointer">Yes</Label></div>
              <div className="flex items-center gap-2"><RadioGroupItem value="no" id="pan-lam-no" /><Label htmlFor="pan-lam-no" className="cursor-pointer">No</Label></div>
            </RadioGroup>
          </div>
  ...
  const renderDeliveryStep = () => (
    <div className="space-y-4">
      ...
      <RadioGroup value={form.deliveryType} onValueChange={(v) => update("deliveryType", v)} className="flex gap-4">
        <div className="flex items-center gap-2"><RadioGroupItem value="pickup" id="pan-del-pickup" /><Label htmlFor="pan-del-pickup" className="cursor-pointer">Pickup from Store</Label></div>
        <div className="flex items-center gap-2"><RadioGroupItem value="delivery" id="pan-del-delivery" /><Label htmlFor="pan-del-delivery" className="cursor-pointer">Home Delivery</Label></div>
      </RadioGroup>
      {form.deliveryType === "delivery" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          <div><Label>Recipient Name *</Label><Input value={form.recipientName} onChange={(e) => update("recipientName", e.target.value)} /></div>
          <div><Label>Phone *</Label><Input value={form.phone} onChange={(e) => update("phone", e.target.value)} /></div>
          <div><Label>House / Flat No.</Label><Input value={form.houseNo} onChange={(e) => update("houseNo", e.target.value)} /></div>
          <div><Label>Address Line 1 *</Label><Input value={form.addressLine1} onChange={(e) => update("addressLine1", e.target.value)} /></div>
          ...
          <div className="md:col-span-2"><Label>Landmark</Label><Input value={form.landmark} onChange={(e) => update("landmark", e.target.value)} /></div>
        </div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      <Card><CardContent className="pt-6">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">Document</span><span>PAN Card</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Print Type</span><span>{form.printType === "pvc" ? "PVC Card" : "Color Print"}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Lamination</span><span>{form.lamination === "yes" ? "Yes" : "No"}</span></div>
  ...
  const renderPreviewStep = () => (
    <Card><CardContent className="pt-6 space-y-3 text-sm">
      ...
      <div className="grid grid-cols-2 gap-y-2">
        ...
        <span className="text-muted-foreground">PAN Number</span><span>{form.panNumber}</span>
        <span className="text-muted-foreground">File</span><span>{form.uploadFile?.name || "—"}</span>
        <span className="text-muted-foreground">Print Type</span><span>{form.printType === "pvc" ? "PVC Card" : "Color Print"}</span>
        <span className="text-muted-foreground">Lamination</span><span>{form.lamination === "yes" ? "Yes" : "No"}</span>
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">PAN Card Print</h1>
          <p className="text-muted-foreground text-sm">Fill in the details to place your order</p>
        </div>
      </div>
      <div className="flex items-center gap-1">
        {steps.map((s, i) => (
          <div key={s.label} className="flex-1"><div className={`h-2 rounded-full transition-colors ${i <= step ? "bg-primary" : "bg-muted"}`} /></div>
        ))}
      </div>
      <div className="flex justify-between text-xs text-muted-foreground">
        {steps.map((s, i) => (<span key={s.label} className={i === step ? "text-primary font-medium" : ""}>{s.label}</span>))}
      </div>
      <Card><CardContent className="pt-6">
      ...
      <div className="flex justify-between">
        <Button variant="outline" onClick={prev} disabled={step === 0}><ArrowLeft className="h-4 w-4 mr-2" /> Previous</Button>
        {step < steps.length - 1 ? (
          <Button onClick={next}>Next <ArrowRight className="h-4 w-4 ml-2" /></Button>
        ) : (
          <Button onClick={handleSubmit} className="bg-gradient-primary">Submit Order</Button>
        )}
      </div>
    </div>
  );
